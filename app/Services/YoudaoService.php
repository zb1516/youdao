...
class YoudaoService
{
    ...
    public function doYoudaoComplete($url,$taskId){
        try{
            ...
            file_put_contents($_SERVER['DOCUMENT_ROOT'].'/batchLog/complete'.time().'.txt',json_encode($postData));
            $ch = curl_init();
    ...
    public function doYoudaoFeedback($url,$data){
        try{
            ...
            file_put_contents($_SERVER['DOCUMENT_ROOT'].'/batchLog/feedback'.time().'.txt',json_encode($postData,JSON_UNESCAPED_UNICODE));
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_POST, 1);// post数据
